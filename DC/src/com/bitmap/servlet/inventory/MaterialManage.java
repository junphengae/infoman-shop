...
import com.bitmap.bean.inventory.InventoryPacking;
import com.bitmap.bean.inventory.SubCategories;
import com.bitmap.bean.inventory.UnitType;
	...
	public void doPost(ReqRes rr) throws ServletException {
		try {
			...
			if (isAction(rr)) {
				...
				if (checkAction(rr, "unit_type_add")) {
					UnitType entity = new UnitType();
					WebUtils.bindReqToEntity(entity, rr.req);
					
					if (UnitType.checkname(entity.getType_name())) {
						
						kson.setError("หน่วยนับซ้ำ");

					}else{
						
						UnitType.insert(entity);
						kson.setSuccess();
					}
				...
				if (checkAction(rr, "unit_type_edit")) {
					UnitType entity = new UnitType();
					WebUtils.bindReqToEntity(entity, rr.req);
					
						if (UnitType.checkname(entity.getType_name())) {
								
								UnitType unit2 = UnitType.selectcheckUpdate(entity.getId(), entity.getType_name());
									
								if (unit2.getCreate_date() != null) {
									
										UnitType.update(entity);
										kson.setSuccess();
				...
				if (checkAction(rr, "add_packing")) {
					InventoryPacking entity = new InventoryPacking();
					WebUtils.bindReqToEntity(entity, rr.req);
					String mat_code = entity.getMat_code();
					String description = entity.getDescription();
					
					
					
					if(InventoryPacking.checkname(description, mat_code)){
						//kson.setError("ชื่อบรรจุภัณฑ์ซ้ำ");
						kson.setSuccess();
						kson.setData("check", "Name");
						//System.out.println("pn::"+entity.getMat_code()+"name::"+entity.getDescription()); 
					}else{
						InventoryPacking.insert(entity);
						kson.setSuccess();
				...
				if (checkAction(rr, "edit_packing")) {
					InventoryPacking entity = new InventoryPacking();
					WebUtils.bindReqToEntity(entity, rr.req);
						String mat_code = entity.getMat_code();
						String description = entity.getDescription();
						String run_id = entity.getRun_id();
						//System.out.println("pn::"+entity.getMat_code()+"name::"+entity.getDescription()); 	
						
						if(InventoryPacking.checkname(description, mat_code)){
							InventoryPacking Packing  =InventoryPacking.selectcheckname(description, mat_code,run_id);
								//System.out.println("create_date:"+Packing.getCreate_date());
								if(Packing.getCreate_date() == null){
									kson.setSuccess();
									kson.setData("check", "Name");
								}else{
									InventoryPacking.update(entity);
									kson.setSuccess();
									kson.setData("check", "success");
								}
