...
public class InventoryPacking {
	...
	public static List<String[]> ddl_th() throws SQLException{
		...
		List<String[]> list = DBUtility.getDropDownListData(conn, tableName, "run_id", "description", "run_id");
		conn.close();
		return list;
	}
	
	public static List<String[]> ddl_th1() throws IllegalAccessException, InvocationTargetException, SQLException, UnsupportedEncodingException{
		//String sql = "SELECT * FROM " + tableName + " WHERE id ='" + id +"'";
		String sql ="SELECT DISTINCT description FROM `inv_packing` WHERE 1";
		Connection conn = DBPool.getConnection();
		...
		while (rs.next()) {
			...
			list.add(new String[]{entity.getDescription(),entity.getDescription()});
		}
		rs.close();
		st.close();
		conn.close();
	...
	public  static boolean checkname(String description ,String mat_code) throws IllegalArgumentException, SQLException, IllegalAccessException, InvocationTargetException{
		InventoryPacking entity = new InventoryPacking();
		entity.setDescription(description);
		entity.setMat_code(mat_code);
		return checkname(entity);
	}
	public  static boolean checkname(InventoryPacking entity) throws IllegalArgumentException, SQLException, IllegalAccessException, InvocationTargetException{
		Connection conn = DBPool.getConnection();
		boolean check = false;
		check = DBUtility.getEntityFromDB(conn, tableName, entity,new String[]{"description","mat_code"});
		conn.close();
		return check;
	}
	public static InventoryPacking selectcheckname(String description,String mat_code,String run_id) throws IllegalArgumentException, SQLException, IllegalAccessException, InvocationTargetException{
		InventoryPacking entity = new InventoryPacking();
		entity.setDescription(description);
		entity.setMat_code(mat_code);
		entity.setRun_id(run_id);
		Connection conn = DBPool.getConnection();
		DBUtility.getEntityFromDB(conn, tableName, entity, new String[]{"description","mat_code","run_id"});
		conn.close();
		return entity;
	}
	
	
	public  static boolean check(InventoryPacking entity) throws IllegalArgumentException, SQLException, IllegalAccessException, InvocationTargetException{
		Connection conn = DBPool.getConnection();
		boolean check = false;
