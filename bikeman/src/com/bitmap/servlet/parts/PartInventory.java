...
public class PartInventory extends ServletUtils {
	...
	public void doPost(ReqRes rr) throws ServletException {
		try {
			if (isAction(rr)) {
				...
				/************************************/
				/*if (checkAction(rr, "stock_in_sn_edit")) {
					EditStockPartNumber(rr);
				}*/
				if (checkAction(rr, "stock_in_non_sn_edit")) {
					EditStockPartNumberNon(rr);
				}
	...
	private void EditStockPartNumberNon(ReqRes rr) throws Exception {
		// TODO EditStockPartNumberNon
		String qty = getParam(rr, "qty");
		String pn = getParam(rr, "pn");
		String create_by = getParam(rr, "create_by");
		//String qty_response = PartMaster.updateInventory(pn, qty, create_by); add
		String qty_response = PartMaster.updateInventoryEditQTY(pn, qty, create_by);
		
		/**
		 * เก็บ Log ในการ add STOCK 
		 */
		...
		PartLot.insertlogEdit(entityPI);
		
		
		kson.setSuccess();
		kson.setData("qty", qty_response);// RETURN QTY ปัจจุบัญ
		rr.out(WebUtils.getResponseString(kson.getJson()));
		
	}

	private void EditStockPartNumber(ReqRes rr) throws Exception {
		// TODO EditStockPartNumber
