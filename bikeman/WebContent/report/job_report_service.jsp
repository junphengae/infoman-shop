...
if(rd_time.equalsIgnoreCase("3")){
	...
	List<serviceServiceBean> list = null;
	list =  jobTS.list_serviceService(paramList);
	 
if (export.equalsIgnoreCase("true")) {

	response.setContentType("application/vnd.ms-excel");
	response.setHeader("Content-Disposition", "attachment; filename=Job_Service" + WebUtils.getDateValue(WebUtils.getCurrentDate()) + ".xls");
...
<title>รายงานการขายบริการ</title>
<body>
	<center>
	<div class="content_head"  >
			<Strong>รายงานการขายบริการ</Strong>
			<br/>
			<Strong>
		<%=HeaderDate %> 
			</Strong>
	...
	<table class="tb" style="width: 100%;">
		<tbody>
			<tr align="center" >
				...
				<th valign="top" align="center" width="7%">Unit Price(฿)</th>
				<th valign="top" align="center" width="7%">Discount(%)</th>
				<th valign="top" align="center" width="7%">Discount Cash</th>
				<th valign="top" align="center" width="7%">Total Discount(฿)</th>
				<th valign="top" align="center" width="7%">Total Vat(฿)</th>
				<th valign="top" align="center" width="10%">Total Amount(฿)</th>					
			</tr>
			<%
			...
			while(ite.hasNext()){
				hasCheck = true;
				serviceServiceBean entity = (serviceServiceBean) ite.next();
				total = Money.add(total, "1"); //	รายการทั้งหมด
				
				
				total_Discount 	+= Double.parseDouble(Money.removeCommas( Money.money(entity.getSrd_dis_total())	));
				total_vat 		+= Double.parseDouble(Money.removeCommas( Money.money(entity.getTotal_vat())		));
				total_price 	+= Double.parseDouble(Money.removeCommas( Money.money(entity.getSrd_net_price())		)); //	รายได้ทั้งหมด 
			%>
			<tr align="center">
				...
				<td style='mso-number-format:"\@"' align="left"><%=entity.getLabor()%>	</td>
				<td style='mso-number-format:"\@"' align="left"><%=entity.getLabor_name()%>	</td>
				<td style='mso-number-format:"\#\,\#\#0\.00"' align="right"><%=(entity.getRate().length()>0 && !(entity.getRate().equalsIgnoreCase("0")|| entity.getRate().equalsIgnoreCase("0.00")))?Money.money(entity.getRate())+"":"0.00"%>		</td>
				...
				<td style='mso-number-format:"\#\,\#\#0\.00"' align="right"><%=(entity.getSrd_dis_total().length()>0 && !(entity.getSrd_dis_total().equalsIgnoreCase("0")|| entity.getSrd_dis_total().equalsIgnoreCase("0.00")))?Money.money(entity.getSrd_dis_total())+"":"0.00"%></td>
				...
				<td style='mso-number-format:"\#\,\#\#0\.00"' align="right"><%=(entity.getSrd_net_price().length()>0 && !(entity.getSrd_net_price().equalsIgnoreCase("0")|| entity.getSrd_net_price().equalsIgnoreCase("0.00")))?Money.money(entity.getSrd_net_price())+"":"0.00"%></td>
