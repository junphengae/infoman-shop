	...
	<div class="wrap_body">
		<div class="body_content">
			<div class="content_head">
				<div class="left">Assign Mechanic เลขที่ใบแจ้งซ่อม: <%=repair_order.getId()%>  | <a class="thickbox btn_box" lang="../info/vehicle_info.jsp?width=400&height=250&vid=<%=vid%>" title="ข้อมูลรถ">ทะเบียนรถ: <%=vehicle.getLicense_plate()%></a> | <a class="thickbox btn_box" lang="../info/customer_info.jsp?width=400&height=250&cus_id=<%=cus_id%>" title="ข้อมูลลูกค้า">ข้อมูลลูกค้า: คุณ<%=customer.getCus_name_th()%></a></div>
				<div class="right">
						<div class="right btn_box" onclick="javascript: window.location='sa_job_reject_repaired_list.jsp?id=<%=id%>&vid=<%=vid%>&cus_id=<%=cus_id%>';">ย้อนกลับ</div>
				</div>
				<div class="clear"></div>
			</div>
					...
					<div class="box_body">
						...
						<tbody>
							...
							<tr id="main_<%=repairLaborTime.getLabor_id()%>_<%=repairLaborTime.getNumber()%>">
								...
								<td align="center">
								<%
								if (repairLaborTime.getStatus().equalsIgnoreCase(RepairLaborTime.STATUS_OPENED_JOB) || repairLaborTime.getStatus().equalsIgnoreCase(RepairLaborTime.STATUS_REJECT)) {
								%>
									<div class="btn_box txt_center thickbox" id="selectMechanic" title="เลือกช่าง" lang="sa_assign_mechanic.jsp?width=850&height=440&modal=false&id=<%=id%>&labor_id=<%=repairLaborTime.getLabor_id()%>&number=<%=repairLaborTime.getNumber()%>">เลือกช่าง</div>
								<%
								} else {
								%>
									ไม่อนุญาต
